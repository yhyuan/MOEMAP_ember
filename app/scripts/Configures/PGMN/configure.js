	...
	*/
	...
	identifyTemplate: [{
		...
		content:'PGMN Well ID: <strong><%= attrs.PGMN_WELL %></strong><br>Conservation Authority: <strong><%= attrs.CONS_AUTHO %></strong><br>\
			County: <strong><%= attrs.COUNTY %></strong><br>\
			Township: <strong><%= attrs.TOWNSHIP %></strong>, <strong><%= attrs.CONCESSION %></strong>, <strong><%= attrs.LOT %></strong><br>\
		...
		Water samples have been collected from this well on <%= attrs.CHEM_CONTE %>. <br><br>By clicking on the Water Chemistry Report link below, you can view the list of \
	...
	identifyTemplate: [{
		...
		content:'Num\u00e9ro du puits du r\u00e9seau: <strong><%= attrs.PGMN_WELL %></strong><br>Office de protection de la nature: <strong><%= attrs.CONS_AUTHO %></strong><br>\
			Comt\u00e9: <strong><%= attrs.COUNTY %></strong><br>\
			Canton: <strong><%= attrs.TOWNSHIP %></strong>, <strong><%= attrs.CONCESSION %></strong>, <strong><%= attrs.LOT %></strong><br>\
		...
		soit dans un laboratoire priv\u00e9.<br><br>Des \u00e9chantillons d\'eau ont \u00e9t\u00e9 pr\u00e9lev\u00e9s dans ce puits les dates suivantes: <%= attrs.CHEM_CONTE %>. <br><br>\
	...
	tableTemplate: '<table id="myTable" class="tablesorter" width="700" border="0" cellpadding="0" cellspacing="1">\
		...
		<% _.each(features, function(feature) {\
			...
			<tr><td><%= attrs.PGMN_WELL %></td><td><%= attrs.LATITUDE %></td><td><%= attrs.LONGITUDE %></td>\
	...
	transformResults: function (results) {
		...
		return _.map(results, function(layer) {
				layer.features = _.map(layer.features, function(feature) {
					...
					feature.attributes["COUNTY"] = wordCapitalize(feature.attributes["COUNTY"]);
					feature.attributes["TOWNSHIP"] = wordCapitalize(feature.attributes["TOWNSHIP"]);
					feature.attributes["CHEM_CONTE"] = getTable(feature.attributes["CHEM_CONTE"]);
					return feature;
				})
				return layer;
			});
	...
	getSearchCondition: function (params) {
		...
		var queryString = createPGMNWellIDQuery(name);
		//this.geocodeWhenQueryFail = false;
		if(queryString !== ''){
			where = queryString;
		} else if (isAquiferTypeSearch(name)) {
			where = "AQUIFER_TY = '" + name +  "'";
		} else if (isWellDepthSearch (name)) {
			var coorsArray = name.split(/\s+/);
			...
			//this.geocodeWhenQueryFail = true;
		}
		return where;
	},
	...
	getSearchSettings: function (params) {
		...
		var settings = {
			...
			geocodeWhenQueryFail: !((createPGMNWellIDQuery(name) !== '') || isAquiferTypeSearch(name) || isWellDepthSearch (name) || (isFromAndTo(name)>0)),
